...
############################ load images for the app
        # image_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "board_images")
        ...
        # self.transperant_board_img = customtkinter.CTkImage(Image.open(os.path.join(image_path, "transperant_board.png")), size=(30, 30))
...
######################### create menu bar
    ...
    #  buttons in menu bar frame
        ...
        # self.transperant_board_button = customtkinter.CTkButton(self.menu_bar_frame, corner_radius=5,width=2, height=5, border_spacing=3, text="",
        #                                               fg_color="transparent", text_color=("gray10", "gray90"), hover_color=("gray70", "gray30"),
        #                                               image=self.transperant_board_img, command=self.transperant_board_button_event)
        # self.transperant_board_button.grid(row=0, column=1, padx=5, pady=5,sticky='we')
...
########################################## transperant frame
        # self.transperant_board_frame = customtkinter.CTkFrame(self, corner_radius=0, fg_color="transparent", )

        # self.canvas1=Canvas(self.transperant_board_frame,bg="green",width=self.s_width-30,height=self.s_height-30,cursor="target", scrollregion=(0,0,self.s_width,self.s_height))
        # self.canvas1.grid(row=1, column=0, sticky="nswe")
        # self.img= ImageTk.PhotoImage(Image.open("D://FYP//PY_WB//fyp-board//board_images//logo.png"))
        # self.canvas1.create_image((10, 10),anchor=NW, image= self.img)
...
# file handling functions
    ...
    def take_snap(self):
        ...
        if req==False:
            ...
            # myScreenshot.save('D://untitled.png')
            im = ImageGrab.grab((0, 0, self.ss_width, self.ss_height))
            self.canvas.delete('all')
            # img=Image.open('D://untitled.png')
...
if __name__ == "__main__":
    app = App()
    app.mainloop()
    # t1=threading.Thread(target=infrared_cap, daemon=True)
    # t1.start()
